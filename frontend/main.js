...
function playLoginSound() {
    try {
        ...
        playTone(523, now,        0.18); // C5
        playTone(659, now + 0.12, 0.18); // E5
        playTone(784, now + 0.24, 0.28); // G5
        playTone(1047, now + 0.4, 0.35); // C6
    } catch (e) {
...
loginForm.addEventListener('submit', async (e) => {
    ...
    if (!email.endsWith('@jiit.ac.in')) {
        showAlert(loginAlert, 'Please use your @jiit.ac.in email address', 'error');
        return;
    }
    ...
    communities.forEach((community, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="community-footer">
                <button class="btn-join" data-community-id="${community.id}" data-community-name="${community.name}">
                    Join <span>&rarr;</span>
                </button>
            </div>
        `;
...
function updateBadgeCounts() {
    ...
    document.getElementById('events-count').textContent = `${upcoming} Upcoming Events`;
}
...
async function loadRecruitment() {
    try {
        const url = currentUser.role === 'admin'
            ...
            : `${API_BASE}/recruitment?email=${encodeURIComponent(currentUser.email)}`;
...
function renderRecruitment() {
    ...
    if (recruitmentResults.length === 0) {
        list.innerHTML = `
            <div class="empty-state">
                ...
                <p>No recruitment results yet</p>
            </div>`;
        return;
    }
    ...
    recruitmentResults.forEach((result, index) => {
        ...
        const expiresDate = new Date(result.expires_at).toLocaleDateString('en-US', {
            month: 'short', day: 'numeric', year: 'numeric'
        });

        const announcedDate = new Date(result.announced_at).toLocaleDateString('en-US', {
        ...
        const statusText = result.status === 'selected' ? 'Selected' : 'Not Selected';

        // Admin sees student name + email, student sees just community
        const metaText = currentUser.role === 'admin'
            ? `${result.student_name} &middot; ${result.student_email}`
        ...
        card.innerHTML = `
            ...
            <div class="recruit-status-badge ${result.status}">
                ${statusIcon}
                ${statusText}
            </div>
        `;
...
function populateRecruitmentTable() {
    ...
    if (recruitmentResults.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:20px">No active results</td></tr>`;
        return;
    }
    ...
    recruitmentResults.forEach((result, index) => {
        const tr = document.createElement('tr');
        tr.style.animationDelay = `${index * 50}ms`;
        tr.dataset.recruitId = result.id;
        ...
        tr.innerHTML = `
            ...
            <td style="font-size:0.8rem;color:var(--text-muted)">${result.student_email}</td>
            <td>${result.community_name}</td>
            <td><span class="event-status ${result.status === 'selected' ? 'ongoing' : 'past'}">${result.status}</span></td>
            <td style="font-size:0.8rem;color:var(--text-muted)">${expiresDate}</td>
            <td>
                <button class="btn-delete" data-recruit-id="${result.id}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="3 6 5 6 21 6"></polyline>
                        <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                    </svg>
    ...
    tbody.querySelectorAll('.btn-delete').forEach(btn => {
        btn.addEventListener('click', () => deleteRecruitment(btn.dataset.recruitId));
    });
}
...
function initAdminForms() {
    ...
    if (addEventForm) {
        addEventForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn     = addEventForm.querySelector('.btn-primary');
            const btnText = btn.querySelector('.btn-text');
            const spinner = btn.querySelector('.spinner');
            const alertEl = document.getElementById('add-event-alert');
            ...
            const formData = {
                ...
                timing:       document.getElementById('event-timing').value,
                venue:        document.getElementById('event-venue').value,
                community_id: parseInt(document.getElementById('event-community').value),
                status:       document.getElementById('event-status').value
            };

            try {
                const res  = await fetch(`${API_BASE}/add-event`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(formData)
                });
                ...
                if (res.ok && data.success) {
                    showAlert(alertEl, 'Event added successfully!', 'success');
                    addEventForm.reset();
                    await loadEvents();
                    populateEventsTable();
                    updateBadgeCounts();
                ...
                showAlert(alertEl, error.message || 'Failed to add event', 'error');
            } finally {
                btnText.classList.remove('hidden');
                spinner.classList.add('hidden');
                btn.disabled = false;
    ...
    // Add Recruitment Result
    const addRecruitForm = document.getElementById('add-recruitment-form');
    if (addRecruitForm) {
        addRecruitForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn     = addRecruitForm.querySelector('.btn-primary');
            const btnText = btn.querySelector('.btn-text');
            const spinner = btn.querySelector('.spinner');
            const alertEl = document.getElementById('add-recruitment-alert');
            ...
            const formData = {
                student_name:  document.getElementById('recruit-name').value,
                student_email: document.getElementById('recruit-email').value,
                community_id:  parseInt(document.getElementById('recruit-community').value),
                status:        document.getElementById('recruit-status').value,
                expires_days:  parseInt(document.getElementById('recruit-expires').value)
            };
            ...
            try {
                ...
                if (res.ok && data.success) {
                    showAlert(alertEl, 'Result announced and notification sent!', 'success');
                    addRecruitForm.reset();
                    await loadRecruitment();
                    populateRecruitmentTable();
                    setTimeout(() => alertEl.classList.add('hidden'), 3000);
                } else {
                    throw new Error(data.detail || 'Failed to add result');
                }
            } catch (error) {
                showAlert(alertEl, error.message || 'Failed to add result', 'error');
            } finally {
                btnText.classList.remove('hidden');
                spinner.classList.add('hidden');
                btn.disabled = false;
    ...
    if (sendNotifForm) {
        sendNotifForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn     = sendNotifForm.querySelector('.btn-primary');
            const btnText = btn.querySelector('.btn-text');
            const spinner = btn.querySelector('.spinner');
            const alertEl = document.getElementById('send-notif-alert');
            ...
            const expiresDays = parseInt(document.getElementById('notif-expires').value);

            try {
                // Get all users to send to based on role
                let targetUsers = [];
                if (role === 'all') {
                    targetUsers = [{ id: null, role: 'student' }, { id: null, role: 'admin' }];
                } else {
                    targetUsers = [{ id: null, role }];
                }

                // Send to all matching users via a broadcast (user_id null = role broadcast)
                ...
                showAlert(alertEl, error.message || 'Failed to send', 'error');
            } finally {
                btnText.classList.remove('hidden');
                spinner.classList.add('hidden');
                btn.disabled = false;
